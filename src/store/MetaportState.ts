...
export const useMetaportStore = create<MetaportState>()((set, get) => ({
    ...
    amount: '',
    setAmount: (amount: string, address: `0x${string}`) => set((state) => {
    ...
    setChainName1: (name: string) => set((state) => {
        // updateState(
        //     name,
        //     state.chainName2
        // )
    ...
    setChainName2: (name: string) => set((state) => {
        const updState = {};
        if (name === MAINNET_CHAIN_NAME) {
            updState['mainnetChain'] = state.mpc.mainnet();
        } else {
    ...
    token: null,
    setToken: (token: dataclasses.TokenData) => set(() => ({
        token: token,
        stepsMetadata: getStepsMetadata(
            get().mpc.config,
            token,
