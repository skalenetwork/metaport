...
export class Action {
    ...
    constructor(
        ...
        this.originAddress = this.mpc.originAddress(
            chainName1, chainName2, token.keyname, token.type);
        ...
        if (this.token.wrapper(this.chainName2)) {
            this.unwrappedToken = mpc.tokenContract(
                chainName1,
                token.keyname,
                token.type,
                provider1
        ...
        const destWrapperAddress = this.mpc.config.connections[this.chainName2][this.token.type][this.token.keyname].chains[this.chainName1].wrapper;
